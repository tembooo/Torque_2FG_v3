...
PRED_XLSX        = os.path.join(BASE_DIR, "predictions_histgbr_optuna.xlsx")
RESULTS_JSON     = os.path.join(BASE_DIR, "histgbr_optuna_results.json")
...
F9_PNG           = os.path.join(BASE_DIR, "actual_vs_pred_F9_histgbr_optuna.png")
FI_PNG           = os.path.join(BASE_DIR, "feature_importance_histgbr_optuna.png")
FI_CSV           = os.path.join(BASE_DIR, "feature_importance_histgbr_optuna.csv")
...
class ZeroCurrentZeroTorqueWrapper(BaseEstimator, RegressorMixin):
    ...
    current_feature: str = "F9"   # جریان
    # هنگام fit: وزن‌دهی بیشتر به نمونه‌های جریان صفر
    ...
    def fit(self, X, y):
        ...
        # وزن‌ها: نمونه‌های F9==0 وزن بیشتر
...
# ------------------------------
# Optuna objective (RepeatedKFold CV, metric: RMSE mean)
# ------------------------------
...
final_model_raw = best_base.fit(X_train, y_train)  # مدل خام بدون قانون
final_model = ZeroCurrentZeroTorqueWrapper(
    base_estimator=best_base,
    current_feature="F9",
    zero_current_weight=5.0
).fit(X_train, y_train)                             # مدل با قانون
...
y_pred_hold_raw = final_model_raw.predict(X_hold)
y_pred_hold_rule = final_model.predict(X_hold)

metrics_raw  = report(y_hold, y_pred_hold_raw)
metrics_rule = report(y_hold, y_pred_hold_rule)

print("Holdout metrics (raw): ", metrics_raw)
print("Holdout metrics (with rule): ", metrics_rule)

# ------------------------------
# Save predictions for whole dataset (with rule)
# ------------------------------
y_all_pred = final_model.predict(X)
out = df.copy()
out["Torque_pred"] = y_all_pred
...
print("Saved models:", BEST_MODEL_RAW, "and", BEST_MODEL_RULE)

# ------------------------------
# Plots
# ------------------------------
# Parity (holdout, with rule)
plt.figure(figsize=(6,6))
mn, mx = min(y_hold.min(), y_pred_hold_rule.min()), max(y_hold.max(), y_pred_hold_rule.max())
plt.scatter(y_hold, y_pred_hold_rule, alpha=0.6)
plt.plot([mn, mx], [mn, mx], "--")
plt.xlabel("Actual Torque")
plt.ylabel("Predicted Torque")
plt.title("Parity Plot (Holdout) | HistGBR+Optuna+Rule")
plt.grid(True); plt.tight_layout()
plt.savefig(PARITY_PNG, dpi=150); plt.close()

# Residuals
resid = y_pred_hold_rule - y_hold
...
plt.scatter(df["F9"], out["Torque_pred"], alpha=0.6, label="Predicted")
plt.xlabel("F9")
plt.ylabel("Torque")
plt.title("Actual vs Predicted Torque by F9 (HistGBR+Optuna+Rule)")
plt.legend(); plt.grid(True); plt.tight_layout()
plt.savefig(F9_PNG, dpi=150); plt.close()

# Feature importance (permutation on holdout) - using raw model to avoid rule masking
...
results = {
    ...
    "holdout_raw": metrics_raw,
    "holdout_with_rule": metrics_rule,
    "paths": {
        "predictions": PRED_XLSX,
        "model_raw": BEST_MODEL_RAW,
        "model_with_rule": BEST_MODEL_RULE,
